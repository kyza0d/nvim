...
---@param mark string
---@return boolean
local function is_lowercase_mark(mark) return 97 <= mark:byte() and mark:byte() <= 122 end

---@param mark string
---@return boolean
local function is_uppercase_mark(mark) return 65 <= mark:byte() and mark:byte() <= 90 end

---@param mark string
---@return boolean
local function is_letter_mark(mark) return is_lowercase_mark(mark) or is_uppercase_mark(mark) end

---@param mark string
---@param bufnr integer
local function delete_mark(mark, bufnr)
  local buffer_marks = marks[bufnr]
  if not buffer_marks or not buffer_marks[mark] then return end
...
local function register_mark(mark, bufnr, line)
  local buffer_marks = marks[bufnr]
  if not buffer_marks then return end
  ...
  if not sign_cache[sign_name] then
    vim.fn.sign_define(sign_name, { text = string.format(' %s', mark), texthl = 'Mark' })
...
local function set_keymaps(bufnr)
  vim.api.nvim_buf_set_keymap(bufnr, 'n', 'm', '', {
    desc = 'Add mark',
    callback = function()
      local ok, mark = pcall(function() return vim.fn.getcharstr() end)
      if not ok or not is_letter_mark(mark) then return end
...
vim.api.nvim_create_autocmd('BufWinEnter', {
  ...
  callback = function(args)
    ...
    if vim.bo[bufnr].bt ~= '' then return true end

    -- Set custom mappings.
    set_keymaps(bufnr)

    if not marks[bufnr] then marks[bufnr] = {} end

    -- Remove all marks that were deleted.
    for mark, _ in pairs(marks[bufnr]) do
      if vim.api.nvim_buf_get_mark(bufnr, mark)[1] == 0 then delete_mark(mark, bufnr) end
    end
